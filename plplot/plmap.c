      ...
      PLFLT minlong, PLFLT maxlong, PLFLT minlat, PLFLT maxlat)
{
     ...
     * read map outline 
     */
    (void) strcpy(filename,type);
    (void) strcat(filename,MAP_FILE);
    ...
    if (io_buffer == NULL) {
	(void) fprintf(stderr, "ran out of memory: plmap\n");
	return;
    }
    error = setvbuf(in,io_buffer,_IOFBF,W_BUFSIZ);
    if (error != 0) {
	(void) fprintf(stderr, "plmap: setvbuf err %d\n",error);
	free(io_buffer);
	return;
    }
#endif
	...
	/* Should test. These do fail! AJB */
	(void) fread(buff, sizeof (unsigned char), 4*n, in);
	if (n == 1) continue;
