...
static char* nexusCharactersDataTypes[] = {
    "standard",
    "dna",
    "rna",
    "nucleotide",
...
static char* nexusCharactersStatesFormat[] = {
    "statespresent",
    "individuals",
    "count",
    "frequency",
...
static char* nexusUnalignedDataTypes[] = {
    "standard",
    "dna",
    "rna",
    "nucleotide",
...
static char* nexusAssumptionsDefType[] = {
    "unord",
    "ord",
    "irrev",
    "irrev.up",
...
static char* nexusAssumptionsPolyTCount[] = {
    "minsteps",
    "maxsteps",
    NULL
};

static char* nexusAssumptionsGapMode[] = {
    "missing",
    "newstate",
    NULL
};

static char* nexusDistancesTriangle[] = {
    "lower",
    "upper",
    "both",
    NULL
...
static AjPNexusCharacters nexusCharactersNew(void)
{
    ...
    ajStrAssC(&thys->DataType, "standard");
    thys->NewTaxa = ajFalse;
    thys->Missing = '?';
    ajStrAssC(&thys->Symbols, "0 1");
    thys->Labels = ajTrue;
...
static AjPNexusAssumptions nexusAssumptionsNew(void)
{
    ...
    ajStrAssC(&thys->DefType, "unord");
    ajStrAssC(&thys->GapMode, "missing");
...
** @param [r] blockname [AjPStr] kBlock name
...
AjBool nexusCommand(AjPList list, AjPStr* command, AjPStr* cmdstr)
{
	...
	else if (incommand)
	{
	    ...
	    {
		ajRegPre(endcommand, &tmpstr);
		ajStrApp(cmdstr, tmpstr);
		return ajTrue;
	    }
	    else {
		if (ajStrLen(*cmdstr))
		    ajStrAppK(cmdstr, '\n');
		ajStrApp(cmdstr, rdline);
	    }
	}
    }
    return ajFalse;
...
static AjBool nexusParseTaxa(AjPNexus thys, AjPList list)
{
	...
	else if (ajStrMatchCaseC(command, "taxlabels"))
	{
	    ...
	    i = nexusGetArray(cmdstr, taxlabexp, 0, &thys->Taxa->TaxLabels);
	    if (i != thys->Taxa->Ntax)
	    {
...
static AjBool nexusParseCharacters(AjPNexus thys, AjPList list, AjBool newtaxa)
{
	...
	{
	    ajStrClean(&cmdstr);
	    ajStrAssS(&thys->Characters->Eliminate, cmdstr);
	}
	else if (ajStrMatchCaseC(command, "taxlabels"))
	{
	    if (!thys->Taxa)
	...
	{
	    i = nexusGetArray(cmdstr, newlinexp, 0, &thys->Characters->Matrix);
	}
...
static AjBool nexusParseUnaligned(AjPNexus thys, AjPList list)
{
	...
	{
	    i = nexusGetArray(cmdstr, newlinexp, 0, &thys->Unaligned->Matrix);
	}
	else
	{
...
static AjBool nexusParseSets(AjPNexus thys, AjPList list)
{
	...
	if (ajStrMatchCaseC(command, "charset"))
	    i = nexusGetArray(cmdstr, newlinexp, 0, &thys->Sets->CharSet);
	else if (ajStrMatchCaseC(command, "stateset"))
	    i = nexusGetArray(cmdstr, newlinexp, 0, &thys->Sets->StateSet);
	else if (ajStrMatchCaseC(command, "changeset"))
	    i = nexusGetArray(cmdstr, newlinexp, 0, &thys->Sets->ChangeSet);
	else if (ajStrMatchCaseC(command, "taxset"))
	    i = nexusGetArray(cmdstr, newlinexp, 0, &thys->Sets->TaxSet);
	else if (ajStrMatchCaseC(command, "treeset"))
	    i = nexusGetArray(cmdstr, newlinexp, 0, &thys->Sets->TreeSet);
	else if (ajStrMatchCaseC(command, "charpartition"))
	    i = nexusGetArray(cmdstr, newlinexp, 0,
...
static AjBool nexusParseTrees(AjPNexus thys, AjPList list)
{
	...
	if (ajStrMatchCaseC(command, "translate"))
	    i = nexusGetArray(cmdstr, newlinexp, 0, &thys->Trees->Translate);
	else if (ajStrMatchCaseC(command, "tree"))
	    i = nexusGetArray(cmdstr, newlinexp, 0, &thys->Trees->Tree);
	else
	{
...
static AjBool nexusParseNotes(AjPNexus thys, AjPList list)
{
	...
	if (ajStrMatchCaseC(command, "text"))
	    i = nexusGetArray(cmdstr, newlinexp, 0, &thys->Notes->Text);
	else if (ajStrMatchCaseC(command, "picture"))
	    i = nexusGetArray(cmdstr, newlinexp, 0, &thys->Notes->Picture);
	else
	{
...
static ajint nexusGetArray(AjPStr src, AjPRegexp exp, ajint isub,
			    AjPStr** dest)
{
    AjPList strlist;
    AjPStr tmpstr = NULL;
...
static AjBool nexusSetSequences(AjPNexus thys)
{
	...
	for (i=0; thys->Taxa->TaxLabels[i]; i++)
	{
	    seqstr = ajStrNewL(thys->Characters->Nchar+1);
    ...
    for (i=0; thys->Characters->Matrix[i]; i++)
    {
	ajStrAssS(&rdline, thys->Characters->Matrix[i]);
	ajStrClean(&rdline);

	if (!taxlabel || thys->Characters->Interleave ||
	    (ajStrLen(seqstr) >= thys->Characters->Nchar))
	{				/* next taxlabel */
	    if (!ajRegExec(word, rdline))
		continue;
	    ajRegSubI(word, 0, &taxlabel);
	    ajRegPost(word, &tmpstr);
	    ajStrAss(&rdline, tmpstr);

	    if (!havetaxa)
	    {
		ajStrAssS(&thys->Taxa->TaxLabels[itax], taxlabel);
		seqstr = ajStrNewL(thys->Characters->Nchar+1);
	...
	if (ajStrLen(thys->Characters->Sequences[i]) !=
	    thys->Characters->Nchar)
	{
