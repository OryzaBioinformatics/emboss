...
static void twofeat_rippledown (AjPFeattable tabA, AjPFeattable tabB,
	ajint overlapi, ajint minrange, ajint maxrange, ajint
	rangetypei, ajint sensei, ajint orderi, AjPFeattable outtab,
...
static void twofeat_find_features (AjPSeq seq, AjPFeattable tab,
	ajint begin, ajint end, AjPStr source, AjPStr type,
	ajint sense, float minscore, float maxscore, AjPStr tag,
	AjPStr value);
static AjBool twofeat_MatchFeature (AjPFeature gf, AjPStr
...
/* go through seq's features adding those that match B to table B */
	...
	if (ajFeattableSize(tabA) && ajFeattableSize(tabB))
	{
		...
		rangetypei, sensei, orderi, outtab, twoout, typeout);

	    /* write features and tidy up */
	    (void) ajReportWrite(report, outtab, seq);        

/* try not deleting - now works OK - not sure why it fails in ajExit if this is included
...
/* check for overlap, minrange, maxrange, rangetype, sense, order */
                    if (twofeat_check_match(gfA, gfB, &detail, overlapi,
                        minrange, maxrange, rangetypei, sensei, orderi)) {
...
/* get feature table of sequence */
    ...
    if (seqtab && seqtab->Features) 
    {
    	...
    	while(ajListIterMore(iter))
    	{
            ...
            /* is this feature local and in the sequence range we want ? */
...
/* could explicitly make a new feature like this, but it is probably safer to let ajFeatCopy do it automatically
**                  gfcopy = ajFeatNew(tab, gf->Source, gf->Type, gf->Start, gf->End,
**                                     gf->Score, gf->Strand, gf->Frame) ;
*/
...
/* check distance criteria */
    if (minrange < maxrange) {	/* ignore distance if these are the same */
        ajDebug("Distance < %d: %B\n", minrange, distance < minrange);
        ajDebug("Distance > %d: %B\n", maxrange, distance > maxrange);
        if (distance < minrange) return ajFalse;
        if (distance > maxrange) return ajFalse;
    }
    ajDebug("Distance OK\n");
...
/* check overlap criteria */
    ...
    case OV_OVERLAP:	/* want sA in B or sB in A */
        if ((sA<sB || sA>eB) && (sB<sA || sB>eA)) return ajFalse;
    break;
    ...
    case OV_NOTIN:	/* want sA or eA out of B and sB or eB out of A */
        if ((sA<=sB && eA>=eB) || (sB<=sA && eB>=eA)) return ajFalse;
    break;

    case OV_AIN:	/* want A in B */
        if (sA<sB || eA>eB) return ajFalse;
    break;

    case OV_BIN:	/* want B in A */
        if (sB<sA || eB>eA) return ajFalse;
    break;
...
/* reject any match found to the same feature (same position, sense and type) */
    if (ss == 0 &&
        ee == 0 &&
...
static ajint twofeat_get_overlap_type (AjPStr overlap)
{
    ...
    if (ajStrMatchC(overlap, "A")) return OV_ANY;
    if (ajStrMatchC(overlap, "O")) return OV_OVERLAP;
    if (ajStrMatchC(overlap, "NO")) return OV_NOTOVER;
    if (ajStrMatchC(overlap, "NW")) return OV_NOTIN;
    if (ajStrMatchC(overlap, "AW")) return OV_AIN;
...
static ajint twofeat_get_range_type (AjPStr rangetype)
{
    ...
    if (ajStrMatchC(rangetype, "N")) return RA_NEAREST;
    if (ajStrMatchC(rangetype, "L")) return RA_LEFT;
    if (ajStrMatchC(rangetype, "R")) return RA_RIGHT;
    if (ajStrMatchC(rangetype, "F")) return RA_FURTHEST;
...
static ajint twofeat_get_sense_type (AjPStr sense)
{
    ...
    if (ajStrMatchC(sense, "A")) return SN_ANY;
    if (ajStrMatchC(sense, "S")) return SN_SAME;
    if (ajStrMatchC(sense, "O")) return SN_OPPOSITE;
...
static ajint twofeat_get_order_type (AjPStr order)
{
    ...
    if (ajStrMatchC(order, "A")) return OR_ANY;
    if (ajStrMatchC(order, "AB")) return OR_AB;
    if (ajStrMatchC(order, "BA")) return OR_BA;
