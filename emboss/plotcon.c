...
int main (int argc, char * argv[]) {
  ...
  float ymin;
  float ymax;
  float contri=0,contrj=0;
  int **matrix;
  int m1=0,m2=0,highindex=0,index;
...
//  showscore   = ajAcdGetInt("showscore");
  ...
  for(i=0;i<numseq;i++){
    ...
    seqcharptr[i] =  ajSeqsetSeq (seqset, i);  /* gets sequence as a string */
    for(j=0;j<ajSeqsetLen(seqset);j++) ajFloat2dPut(&score,i,j,0.);
  }
  
  for(bin=0; bin<numbins; bin++) sumscore[bin] = 0.;
...
/************** Loop over the sequence set length **************/
  for(k=0; k< ajSeqsetLen(seqset); k++){
   ...
   /* Generate a score for each column */
    for(i=0;i<numseq;i++){
      m1 = ajSeqCvtK(cvt, seqcharptr[i][k]);  /* gets int code from conversion table */
...
/*************** End of Loop ***************/
  ...
  for(bin=0;bin<numbins;bin++){
   sumscore[bin] = sumscore[bin]/((float)(numseq*(numseq-1)*winsize*winsize));
  }
  ...
  if(!text){
   ...
   /* print out results */
   ajFmtPrintF(outf,"##2D Plot\n##Title Similarity Plot of Aligned Sequences\n");
   ...
   ajFmtPrintF(outf,"##ScaleXmin %f ScaleXmax %f "
               "ScaleYmin %f ScaleYmax %f\n",0.,(float)ajSeqsetLen(seqset),ymin,ymax);
