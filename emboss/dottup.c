...
/*	tickgap = acceptableticksx[13];  bugfixed AJB 26/7/00 */
	tickgap = acceptableticksx[10];
      ...
      if(ajSeqLen(seq2)/ajSeqLen(seq1) > 10 ){              /* alot smaller then just label start and end */
	ajGraphLine(0.0,0.0,0.0,0.0-ticklen);
	sprintf(ptr,"%d",ajSeqOffset(seq1));
	ajGraphTextMid ( 0.0,0.0-(onefifth),ptr);
	
	ajGraphLine((float)(ajSeqLen(seq1)),0.0,(float)ajSeqLen(seq1),0.0-ticklen);
	...
	i++;
      ...
      if(ajSeqLen(seq1)/ajSeqLen(seq2) > 10 ){              /* alot smaller then just label start and end */
    ...
    ajFmtPrintF(outfile, "%10s %10s Length\n",
		ajSeqName(seq1), ajSeqName(seq2));
    if(matchlist) 
      embWordMatchListPrint(outfile, matchlist);
