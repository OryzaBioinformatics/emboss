...
int main(int argc, char **argv)
{
  ...
  while(ajFileReadLine(inf,&line))
    if(strstr(ajStrStr(line),"REBASE codes")) break;
	
  if(!ajFileReadLine(inf,&line)) ajFatal("No Supplier Information Found");
  if(!ajFileReadLine(inf,&line)) ajFatal("Unexpected EOF");
  while(ajStrLen(line))
    {
      ajStrClean(&line);
      ajFmtPrintF(outf3,"%s\n",ajStrStr(line));
  ...
  while(ajFileReadLine(inf,&line))
    {
      /* Get RE */
      if(!ajStrPrefixC(line,"<1>")) continue;
      ajStrAssC(&code,ajStrStr(line)+3);
      /* Get isoschizomers */
      if(!ajFileReadLine(inf,&line)) ajFatal("Unexpected EOF");
      if(ajStrLen(line)>3) ajStrAssC(&isoschiz,ajStrStr(line)+3);
      else ajStrAssC(&isoschiz,"");
      /* Get recognition sequence */
      if(!ajFileReadLine(inf,&line)) ajFatal("Unexpected EOF");
      if(ajStrLen(line)>3) ajStrAssC(&pattern,ajStrStr(line)+3);
      else ajStrAssC(&pattern,"");
      /* Get methylation */
      if(!ajFileReadLine(inf,&line)) ajFatal("Unexpected EOF");
      if(ajStrLen(line)>3) ajStrAssC(&meth,ajStrStr(line)+3);
      else ajStrAssC(&meth,"");
      /* Get title */
      if(!ajFileReadLine(inf,&line)) ajFatal("Unexpected EOF");
      if(ajStrLen(line)>3) ajStrAssC(&tit,ajStrStr(line)+3);
      else ajStrAssC(&tit,"");
      /* Get source */
      if(!ajFileReadLine(inf,&line)) ajFatal("Unexpected EOF");
      if(ajStrLen(line)>3) ajStrAssC(&sou,ajStrStr(line)+3);
      else ajStrAssC(&sou,"");
      ...
      if(ajStrLen(line)>3)
	{
      ...
      while(ajStrLen(line))
	{
	  if(!ajFileReadLine(inf,&line)) break;
	  ++count;
	}
      ...
      else
	{
      ...
      process_pattern(&pattern,&code,outf,hassup);
