...
int main (int argc, char * argv[]) {
  ...
  */
  ...
  gscore = EALIGN(ajStrStr(aa0str),ajStrStr(aa1str),ajSeqLen(seq),ajSeqLen(seq2),
		 (gdelval-ggapval),ggapval,res,&nres);

  nc=calcons(ajStrStr(aa0str),ajSeqLen(seq),ajStrStr(aa1str),ajSeqLen(seq2),res);
...
{ 
  ...
  ck = CHECK_SCORE((unsigned char *)A,(unsigned char *)B,M,N,S,NC);
